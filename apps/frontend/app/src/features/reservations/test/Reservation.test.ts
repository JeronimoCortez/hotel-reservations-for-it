...
vi.mock("../services/ReservationService", () => ({
  reservationService: {
    ...
    create: vi.fn((payload) => Promise.resolve({ ...payload, id: "2", status: "PENDING" })),
    cancel: vi.fn(() => Promise.resolve({ reservationId: "1", status: "CANCELLED" })),
    confirm: vi.fn(() => Promise.resolve({ reservationId: "1", status: "CONFIRMED" })),
  },
}));
...
describe("ReservationStore", () => {
  ...
  it("createReservation adds a new reservation", async () => {
    await act(async () => {
      useReservationStore.getState().setToken("token");
